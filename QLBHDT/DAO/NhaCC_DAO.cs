...
namespace DAO
{
    public class NhaCC_DAO
    {
        ...
        {
            conn = DataProvider.KetNoi();
            SqlDataAdapter adapter = new SqlDataAdapter("NhaCC_SelectAll", conn);
            DataTable table = new DataTable();
            adapter.Fill(table);
            adapter.Dispose();
            return table;
        }
        public static DataTable GetByName(NhaCC_DTO nhaccDTO)
        {
            cmd = new SqlCommand("NhaCC_GetByName", conn);
            cmd.CommandType = CommandType.StoredProcedure;
            cmd.Parameters.Add("@TenNhaCC", SqlDbType.NVarChar).Value = nhaccDTO.TenNhaCC;
